...
HANDLE intel_driver::Load() {
	...
	if (intel_driver::IsRunning()) {
		//////Log[faatal-mapper]  \\Device\\Nal is already in use." << std::endl);
	...
	////Log[faatal-mapper] loading vulnerable driver " << std::endl);

	std::wstring driver_path = GetDriverPath();
	if (driver_path.empty()) {
		////Log[faatal-mapper]  Can't find TEMP folder" << std::endl);
		return INVALID_HANDLE_VALUE;
	}
	...
	if (!utils::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(intel_driver_resource::driver), sizeof(intel_driver_resource::driver))) {
		////Log[faatal-mapper]  Failed to create vulnerable driver file" << std::endl);
		return INVALID_HANDLE_VALUE;
	}

	if (!service::RegisterAndStart(driver_path)) {
		////Log[faatal-mapper]  Failed to register and start service for the vulnerable driver" << std::endl);
		_wremove(driver_path.c_str());
		return INVALID_HANDLE_VALUE;
	}
	...
	{
		////Log[faatal-mapper]  Failed to load driver iqvw64e.sys" << std::endl);
	...
	if (ntoskrnlAddr == 0) {
		//Log[faatal-mapper]  Failed to get ntoskrnl.exe" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearPiDDBCacheTable(result)) {
		//	Log[faatal-mapper]  Failed to ClearPiDDBCacheTable" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearKernelHashBucketList(result)) {
		//Log[faatal-mapper]  Failed to ClearKernelHashBucketList" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearMmUnloadedDrivers(result)) {
		//Log[!] Failed to ClearMmUnloadedDrivers" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}
...
bool intel_driver::Unload(HANDLE device_handle) {
	//Log[<] Unloading vulnerable driver" << std::endl);
	...
	if (!file_ofstream.write((char*)randomData, newFileLen)) {
		//Log[!] Error dumping shit inside the disk" << std::endl);
	}
	else {
		//Log[+] Vul driver data destroyed before unlink" << std::endl);
	}
	file_ofstream.close();
	delete[] randomData;
...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size) {
	...
	if (!GetPhysicalAddress(device_handle, address, &physical_address)) {
		//Log[faatal-mapper]  Failed to translate virtual address 0x" << reinterpret_cast<void*>(address) << std::endl);
		return false;
	}
	...
	if (!mapped_physical_memory) {
		//Log[faatal-mapper]  Failed to map IO space of 0x" << reinterpret_cast<void*>(physical_address) << std::endl);
		return false;
	}
	...
	if (!UnmapIoSpace(device_handle, mapped_physical_memory, size))
		//Log[!] Failed to unmap IO space of physical address 0x" << reinterpret_cast<void*>(physical_address) << std::endl);
#endif
...
uint64_t intel_driver::MmAllocateIndependentPagesEx(HANDLE device_handle, uint32_t size)
{
	...
	if (!kernel_MmAllocateIndependentPagesEx)
	{
		...
		if (!kernel_MmAllocateIndependentPagesEx) {
			//Log[!] Failed to find MmAllocateIndependentPagesEx" << std::endl);
			return 0;
		}
...
bool intel_driver::MmFreeIndependentPages(HANDLE device_handle, uint64_t address, uint32_t size)
{
	...
	if (!kernel_MmFreeIndependentPages)
	{
		...
		if (!kernel_MmFreeIndependentPages) {
			//Log[!] Failed to find MmFreeIndependentPages" << std::endl);
			return false;
		}
...
BOOLEAN intel_driver::MmSetPageProtection(HANDLE device_handle, uint64_t address, uint32_t size, ULONG new_protect)
{
	if (!address)
	{
		//Log[!] Invalid address passed to MmSetPageProtection" << std::endl);
		return FALSE;
	}
	...
	if (!kernel_MmSetPageProtection)
	{
		...
		if (!kernel_MmSetPageProtection) {
			//Log[!] Failed to find MmSetPageProtection" << std::endl);
			return FALSE;
		}
...
uint64_t intel_driver::MmAllocatePagesForMdl(HANDLE device_handle, LARGE_INTEGER LowAddress, LARGE_INTEGER HighAddress, LARGE_INTEGER SkipBytes, SIZE_T TotalBytes)
{
	...
	{
		//Log[!] Failed to find MmAlocatePagesForMdl" << std::endl);
		return 0;
	}
...
uint64_t intel_driver::MmMapLockedPagesSpecifyCache(HANDLE device_handle, uint64_t pmdl, ntt::KPROCESSOR_MODE AccessMode, ntt::MEMORY_CACHING_TYPE CacheType, uint64_t RequestedAddress, ULONG BugCheckOnFailure, ULONG Priority)
{
	...
	{
		//Log[!] Failed to find MmMapLockedPagesSpecifyCache" << std::endl);
		return 0;
	}
...
bool intel_driver::MmProtectMdlSystemAddress(HANDLE device_handle, uint64_t MemoryDescriptorList, ULONG NewProtect)
{
	...
	{
		//Log[!] Failed to find MmProtectMdlSystemAddress" << std::endl);
		return 0;
	}
...
bool intel_driver::MmUnmapLockedPages(HANDLE device_handle, uint64_t BaseAddress, uint64_t pmdl)
{
	...
	{
		//Log[!] Failed to find MmUnmapLockedPages" << std::endl);
		return 0;
	}
...
bool intel_driver::MmFreePagesFromMdl(HANDLE device_handle, uint64_t MemoryDescriptorList)
{
	...
	{
		//Log[!] Failed to find MmFreePagesFromMdl" << std::endl);
		return 0;
	}
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, ntt::POOL_TYPE pool_type, uint64_t size) {
	if (!size)
		return 0;
	...
	if (!kernel_ExAllocatePool) {
		//Log[!] Failed to find ExAllocatePool" << std::endl);
		return 0;
	}
...
bool intel_driver::ClearMmUnloadedDrivers(HANDLE device_handle) {
	...
	NTSTATUS status = NtQuerySystemInformation(static_cast<SYSTEM_INFORMATION_CLASS>(ntt::SystemExtendedHandleInformation), buffer, buffer_size, &buffer_size);

	while (status == ntt::STATUS_INFO_LENGTH_MISMATCH)
	{
		VirtualFree(buffer, 0, MEM_RELEASE);

		buffer = VirtualAlloc(nullptr, buffer_size, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		status = NtQuerySystemInformation(static_cast<SYSTEM_INFORMATION_CLASS>(ntt::SystemExtendedHandleInformation), buffer, buffer_size, &buffer_size);
	}
	...
	auto system_handle_inforamtion = static_cast<ntt::PSYSTEM_HANDLE_INFORMATION_EX>(buffer);

	for (auto i = 0u; i < system_handle_inforamtion->HandleCount; ++i)
	{
		const ntt::SYSTEM_HANDLE current_system_handle = system_handle_inforamtion->Handles[i];
	...
	if (!ReadMemory(device_handle, object + 0x8, &device_object, sizeof(device_object)) || !device_object) {
		//Log[!] Failed to find device_object" << std::endl);
		return false;
	}
	...
	if (!ReadMemory(device_handle, device_object + 0x8, &driver_object, sizeof(driver_object)) || !driver_object) {
		//Log[!] Failed to find driver_object" << std::endl);
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_object + 0x28, &driver_section, sizeof(driver_section)) || !driver_section) {
		//Log[!] Failed to find driver_section" << std::endl);
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name)) || us_driver_base_dll_name.Length == 0) {
		//Log[!] Failed to find driver name" << std::endl);
		return false;
	}
	...
	if (!ReadMemory(device_handle, (uintptr_t)us_driver_base_dll_name.Buffer, unloadedName.get(), us_driver_base_dll_name.Length)) {
		//Log[!] Failed to read driver name" << std::endl);
		return false;
	}
	...
	if (!WriteMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name))) {
		//Log[!] Failed to write driver name length" << std::endl);
		return false;
	}

	//Log[+] MmUnloadedDrivers Cleaned: " << unloadedName << std::endl);
	return true;
}
...
bool intel_driver::ExAcquireResourceExclusiveLite(HANDLE device_handle, PVOID Resource, BOOLEAN wait) {
	...
	if (!kernel_ExAcquireResourceExclusiveLite) {
		//Log[!] Failed to find ExAcquireResourceExclusiveLite" << std::endl);
		return 0;
	}
...
bool intel_driver::ExReleaseResourceLite(HANDLE device_handle, PVOID Resource) {
	...
	if (!kernel_ExReleaseResourceLite) {
		//Log[!] Failed to find ExReleaseResourceLite" << std::endl);
		return false;
	}
...
BOOLEAN intel_driver::RtlDeleteElementGenericTableAvl(HANDLE device_handle, PVOID Table, PVOID Buffer) {
	...
	if (!kernel_RtlDeleteElementGenericTableAvl) {
		//Log[!] Failed to find RtlDeleteElementGenericTableAvl" << std::endl);
		return false;
	}
...
PVOID intel_driver::RtlLookupElementGenericTableAvl(HANDLE device_handle, PRTL_AVL_TABLE Table, PVOID Buffer) {
	...
	if (!kernel_RtlDeleteElementGenericTableAvl) {
		//Log[!] Failed to find RtlLookupElementGenericTableAvl" << std::endl);
		return nullptr;
	}
...
bool intel_driver::ClearPiDDBCacheTable(HANDLE device_handle) { //PiDDBCacheTable added on LoadDriver
	...
	if (PiDDBLockPtr == NULL) { // PiDDBLock pattern changes a lot from version 1607 of windows and we will need a second pattern if we want to keep simple as posible
		...
		if (PiDDBLockPtr == NULL) {
			//Log[faatal-mapper]  Warning PiDDBLock not found" << std::endl);
			return false;
		}
		//Log[+] PiDDBLock found with second pattern" << std::endl);
	...
	if (PiDDBCacheTablePtr == NULL) {
		//Log[faatal-mapper]  Warning PiDDBCacheTable not found" << std::endl);
		return false;
	}

	//Log("[ethic]: piddblock:  0x" << std::hex << PiDDBLockPtr << std::endl);
	//Log("[ethic]: cachetable: 0x" << std::hex << PiDDBCacheTablePtr << std::endl);
	...
	if (!ExAcquireResourceExclusiveLite(device_handle, PiDDBLock, true)) {
		//Log[faatal-mapper]  Can't lock PiDDBCacheTable" << std::endl);
		return false;
	}
	//Log[+] PiDDBLock Locked" << std::endl);
	...
	if (pFoundEntry == nullptr) {
		//Log[faatal-mapper]  Not found in cache" << std::endl);
	...
	if (!RtlDeleteElementGenericTableAvl(device_handle, PiDDBCacheTable, pFoundEntry)) {
		//Log[faatal-mapper]  Can't delete from PiDDBCacheTable" << std::endl);
		ExReleaseResourceLite(device_handle, PiDDBLock);
		return false;
	}
	...
	//Log[+] PiDDBCacheTable Cleaned" << std::endl);
...
uintptr_t intel_driver::FindPatternAtKernel(HANDLE device_handle, uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, const char* szMask) {
	if (!dwAddress) {
		//Log[faatal-mapper]  No module address to find pattern" << std::endl);
		return 0;
	}

	if (dwLen > 1024 * 1024 * 1024) { //if read is > 1GB
		//Log[faatal-mapper]  Can't find pattern, Too big section" << std::endl);
		return 0;
	}

	auto sectionData = std::make_unique<BYTE[]>(dwLen);
	if (!ReadMemory(device_handle, dwAddress, sectionData.get(), dwLen)) {
		//Log[faatal-mapper]  Read failed in FindPatternAtKernel" << std::endl);
		return 0;
	}
	...
	if (result <= 0) {
		//Log[faatal-mapper]  Can't find pattern" << std::endl);
		return 0;
	}
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	...
	if (!ci) {
		//Log[faatal-mapper]  Can't Find ci.dll module address" << std::endl);
		return false;
	}
	...
	if (!sig) {
		//Log[faatal-mapper]  Can't Find g_KernelHashBucketList" << std::endl);
		return false;
	}
	auto sig2 = FindPatternAtKernel(device_handle, (uintptr_t)sig - 50, 50, PUCHAR("\x48\x8D\x0D"), "xxx");
	if (!sig2) {
		//Log[faatal-mapper]  Can't Find g_HashCacheLock" << std::endl);
		return false;
	}
	...
	{
		//Log[faatal-mapper]  Can't Find g_HashCache relative address" << std::endl);
		return false;
	}

	//Log[+] g_KernelHashBucketList Found 0x" << std::hex << g_KernelHashBucketList << std::endl);

	if (!ExAcquireResourceExclusiveLite(device_handle, g_HashCacheLock, true)) {
		//Log[faatal-mapper]  Can't lock g_HashCacheLock" << std::endl);
		return false;
	}
	//Log[+] g_HashCacheLock Locked" << std::endl);
	...
	if (!ReadMemory(device_handle, (uintptr_t)prev, &entry, sizeof(entry))) {
		//Log[faatal-mapper]  Failed to read first g_KernelHashBucketList entry!" << std::endl);
		if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
			//Log[faatal-mapper]  Failed to release g_KernelHashBucketList lock!" << std::endl);
		}
		return false;
	}
	if (!entry) {
		//Log[!] g_KernelHashBucketList looks empty!" << std::endl);
		if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
			//Log[faatal-mapper]  Failed to release g_KernelHashBucketList lock!" << std::endl);
		}
		return true;
	...
	while (entry) {
		...
		if (!ReadMemory(device_handle, (uintptr_t)entry + offsetof(HashBucketEntry, DriverName.Length), &wsNameLen, sizeof(wsNameLen)) || wsNameLen == 0) {
			//Log[faatal-mapper]  Failed to read g_KernelHashBucketList entry text len!" << std::endl);
			if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
				//Log[faatal-mapper]  Failed to release g_KernelHashBucketList lock!" << std::endl);
			}
			return false;
		...
		if (expected_len == wsNameLen) {
			...
			if (!ReadMemory(device_handle, (uintptr_t)entry + offsetof(HashBucketEntry, DriverName.Buffer), &wsNamePtr, sizeof(wsNamePtr)) || !wsNamePtr) {
				//Log[faatal-mapper]  Failed to read g_KernelHashBucketList entry text ptr!" << std::endl);
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					//Log[faatal-mapper]  Failed to release g_KernelHashBucketList lock!" << std::endl);
				}
				return false;
			...
			if (!ReadMemory(device_handle, (uintptr_t)wsNamePtr, wsName.get(), wsNameLen)) {
				//Log[faatal-mapper]  Failed to read g_KernelHashBucketList entry text!" << std::endl);
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					//Log[faatal-mapper]  Failed to release g_KernelHashBucketList lock!" << std::endl);
				}
				return false;
			...
			if (find_result != std::wstring::npos) {
				//Log[+] Found In g_KernelHashBucketList: " << std::wstring(&wsName[find_result]) << std::endl);
				HashBucketEntry* Next = 0;
				if (!ReadMemory(device_handle, (uintptr_t)entry, &Next, sizeof(Next))) {
					//Log[faatal-mapper]  Failed to read g_KernelHashBucketList next entry ptr!" << std::endl);
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						//Log[faatal-mapper]  Failed to release g_KernelHashBucketList lock!" << std::endl);
					}
					return false;
				...
				if (!WriteMemory(device_handle, (uintptr_t)prev, &Next, sizeof(Next))) {
					//Log[faatal-mapper]  Failed to write g_KernelHashBucketList prev entry ptr!" << std::endl);
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						//Log[faatal-mapper]  Failed to release g_KernelHashBucketList lock!" << std::endl);
					}
					return false;
				...
				if (!FreePool(device_handle, (uintptr_t)entry)) {
					//Log[faatal-mapper]  Failed to clear g_KernelHashBucketList entry pool!" << std::endl);
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						//Log[faatal-mapper]  Failed to release g_KernelHashBucketList lock!" << std::endl);
					}
					return false;
				}
				//Log[+] g_KernelHashBucketList Cleaned" << std::endl);
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					//Log[faatal-mapper]  Failed to release g_KernelHashBucketList lock!" << std::endl);
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						//Log[faatal-mapper]  Failed to release g_KernelHashBucketList lock!" << std::endl);
		...
		if (!ReadMemory(device_handle, (uintptr_t)entry, &entry, sizeof(entry))) {
			//Log[faatal-mapper]  Failed to read g_KernelHashBucketList next entry!" << std::endl);
			if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
				//Log[faatal-mapper]  Failed to release g_KernelHashBucketList lock!" << std::endl);
			}
			return false;
