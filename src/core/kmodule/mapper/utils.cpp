...
std::wstring utils::GetFullTempPath() {
	...
	if (!get_temp_path_ret || get_temp_path_ret > MAX_PATH + 1) {
		//Log[-] Failed to get temp path" << std::endl);
		return L"";
	}
...
uint64_t utils::GetKernelModuleAddress(const std::string& module_name) {
	...
	NTSTATUS status = NtQuerySystemInformation(static_cast<SYSTEM_INFORMATION_CLASS>(ntt::SystemModuleInformation), buffer, buffer_size, &buffer_size);
	...
	while (status == ntt::STATUS_INFO_LENGTH_MISMATCH) {
		...
		status = NtQuerySystemInformation(static_cast<SYSTEM_INFORMATION_CLASS>(ntt::SystemModuleInformation), buffer, buffer_size, &buffer_size);
	}
	...
	const auto modules = static_cast<ntt::PRTL_PROCESS_MODULES>(buffer);
	if (!modules)
		return 0;
