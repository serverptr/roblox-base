...
uint64_t kdmapper::AllocMdlMemory(HANDLE iqvw64e_device_handle, uint64_t size, uint64_t* mdlPtr) {
	...
	if (!mdl) {
		////////Log[-] Can't allocate pages for mdl" << std::endl);
		return { 0 };
	}

	uint32_t byteCount = 0;
	if (!intel_driver::ReadMemory(iqvw64e_device_handle, mdl + 0x028 /*_MDL : byteCount*/, &byteCount, sizeof(uint32_t))) {
		////////Log[-] Can't read the _MDL : byteCount" << std::endl);
		return { 0 };
	}

	if (byteCount < size) {
		////////Log[-] Couldn't allocate enough memory, cleaning up" << std::endl);
		intel_driver::MmFreePagesFromMdl(iqvw64e_device_handle, mdl);
		intel_driver::FreePool(iqvw64e_device_handle, mdl);
		return { 0 };
	}

	auto mappingStartAddress = intel_driver::MmMapLockedPagesSpecifyCache(iqvw64e_device_handle, mdl, ntt::KernelMode, ntt::MmCached, NULL, FALSE, ntt::NormalPagePriority);
	if (!mappingStartAddress) {
		////////Log[-] Can't set mdl pages cache, cleaning up." << std::endl);
		intel_driver::MmFreePagesFromMdl(iqvw64e_device_handle, mdl);
		intel_driver::FreePool(iqvw64e_device_handle, mdl);
		return { 0 };
	}
	...
	if (!result) {
		////////Log[-] Can't change protection for mdl pages, cleaning up" << std::endl);
		intel_driver::MmUnmapLockedPages(iqvw64e_device_handle, mappingStartAddress, mdl);
		intel_driver::MmFreePagesFromMdl(iqvw64e_device_handle, mdl);
		intel_driver::FreePool(iqvw64e_device_handle, mdl);
		return { 0 };
	}
	//////Log[+] Allocated pages for mdl" << std::endl);
...
uint64_t kdmapper::AllocIndependentPages(HANDLE device_handle, uint32_t size)
{
	...
	if (!base)
	{
		//////Log[-] Error allocating independent pages" << std::endl);
		return 0;
	}
	...
	if (!intel_driver::MmSetPageProtection(device_handle, base, size, PAGE_EXECUTE_READWRITE))
	{
		...
		// 
		// 
		//  Failed to change page protections" << std::endl);
...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {
	...
	if (mode == AllocationMode::AllocateMdl) {
		...
		kernel_image_base = intel_driver::AllocatePool(iqvw64e_device_handle, ntt::POOL_TYPE::NonPagedPool, image_size);
	}
	...
	do {
		...
		if (destroyHeader) {
			...
			//////Log[+] Skipped 0x" << std::hex << TotalVirtualHeaderSize << L" bytes of PE Header" << std::endl);
		}
		...
		{
			//////Log[-] Failed to fix cookie" << std::endl);
			return 0;
		}
		...
		if (callback) {
			if (!callback(&param1, &param2, realBase, image_size, mdlptr)) {
				//////Log[-] Callback returns false, failed!" << std::endl);
				kernel_image_base = realBase;
				break;
			}
		}
		...
		if (free) {
			...
			if (free_status) {
				//////Log[+] Memory has been released" << std::endl);
			}
			else {
				//////Log[-] WARNING: Failed to free memory!" << std::endl);
			}
		}
...
bool kdmapper::FixSecurityCookie(void* local_image, uint64_t kernel_image_base)
{
	...
	{
		//////Log[+] Load config directory wasn't found, probably StackCookie not defined, fix cookie skipped" << std::endl);
		return true;
	}
	...
	if (!stack_cookie)
	{
		//////Log[+] StackCookie not defined, fix cookie skipped" << std::endl);
	...
	if (*(uintptr_t*)(stack_cookie) != 0x2B992DDFA232) {
		//////Log[-] StackCookie already fixed!? this probably wrong" << std::endl);
		return false;
	}

	//////Log[+] Fixing stack cookie" << std::endl);
