...
namespace intel_driver
{
	...
	uint64_t AllocatePool(HANDLE device_handle, ntt::POOL_TYPE pool_type, uint64_t size);
	/*added by psec*/
	...
	uint64_t MmMapLockedPagesSpecifyCache(HANDLE device_handle, uint64_t pmdl, ntt::KPROCESSOR_MODE AccessMode, ntt::MEMORY_CACHING_TYPE CacheType, uint64_t RequestedAddress, ULONG BugCheckOnFailure, ULONG Priority);
	...
	bool CallKernelFunction(HANDLE device_handle, T* out_result, uint64_t kernel_function_address, const A ...arguments) {
		...
		if (ntdll == 0) {
			//Log[-] Failed to load ntdll.dll" << std::endl); //never should happens
			return false;
		}
		...
		if (!NtAddAtom)
		{
			//Log[-] Failed to get export ntdll.NtAddAtom" << std::endl);
			return false;
		}
		...
		*(uint64_t*)&kernel_injected_jmp[2] = kernel_function_address;
		...
		if (original_kernel_function[0] == kernel_injected_jmp[0] &&
			...
			//Log[-] FAILED!: The code was already hooked!! another instance of kdmapper running?!" << std::endl);
			return false;
		}
