...
bool service::RegisterAndStart(const std::wstring& driver_path) {
	...
	//	Log("[ethic]: faatal-drv status: 0x" << std::hex << Status << std::endl);
	...
	if (Status == 0xC0000603) { //STATUS_IMAGE_CERT_REVOKED
		...
		Log("[ethic]: vuln_drv error, check #fixes " << std::endl);
	}
...
bool service::StopAndRemove(const std::wstring& driver_name) {
	...
	//Log("[ethic]: faatal-drv unload status: 0x" << std::hex << st << std::endl);
	if (st != 0x0) {
		Log("[ethic]: failed to unload" << std::endl);
